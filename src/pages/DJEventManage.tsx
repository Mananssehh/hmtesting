            ...
            {broadcastNowPlaying ? (
              <div className="space-y-3">
                <div className="flex items-start gap-3">
                  {broadcastNowPlaying.album_art ? (
                    <img
                      src={broadcastNowPlaying.album_art}
                      alt={broadcastNowPlaying.title}
                      className="h-16 w-16 rounded-lg object-cover border border-border/60"
                    />
                  ) : (
                    <div className="h-16 w-16 rounded-lg bg-secondary/40 flex items-center justify-center">
                      <Music className="h-6 w-6 text-muted-foreground" />
                    </div>
                  )}
                  <div className="min-w-0 flex-1">
                    <div className="font-semibold truncate">{broadcastNowPlaying.title}</div>
                    {broadcastNowPlaying.artist && (
                      <div className="text-sm text-muted-foreground truncate">{broadcastNowPlaying.artist}</div>
                    )}
                    <div className="text-[10px] uppercase tracking-wider text-muted-foreground/80 mt-1">
                      {broadcastNowPlaying.source ?? "broadcast"}
                      {broadcastNowPlaying.updated_at && (
                        <> · {new Date(broadcastNowPlaying.updated_at).toLocaleTimeString()}</>
                      )}
                    </div>
                  </div>
                </div>
